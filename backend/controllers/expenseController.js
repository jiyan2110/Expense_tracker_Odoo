...
exports.createExpense = async (req, res) => {
  try {
    ...
    try {
      amountInCompanyCurrency = await currencyService.convertToCompanyCurrency(Number(amount), currency, company.defaultCurrency);
    } catch (e) {
      // fallback: keep original amount in amountInCompanyCurrency
    }

    // build approver sequence
    let approverIds = [];
    ...
    // allow admin to pass explicit approvers JSON array
    if (approvers) {
      let parsed = [];
      try { parsed = JSON.parse(approvers); } catch { parsed = Array.isArray(approvers) ? approvers : []; }
      parsed.forEach(a => { if (!approverIds.includes(a)) approverIds.push(a); });
    }

    // fallback: find all managers in company (simple fallback)
    if (approverIds.length === 0) {
      const managers = await User.find({ companyId: req.user.companyId, role: 'Manager' }).select('_id');
      approverIds = managers.map(m => m._id);
    }
...
exports.listExpenses = async (req, res) => {
  try {
    ...
    if (pendingForMe === 'true') {
      // fetch expenses where current approver is this user
      const list = await Expense.find({
        status: 'Pending',
        approvers: req.user.id,
        $expr: { $eq: ["$approvers.$[idx]", req.user.id] } // not directly supported; use filter below
      }).lean();

      // because expression above won't work in all mongo versions, fallback:
      const all = await Expense.find({ status: 'Pending', approvers: { $in: [req.user.id] } }).populate('userId', 'name email');
      const filtered = all.filter(e => {
        // is current approver?
        const idx = e.currentApproverIndex || 0;
        return String(e.approvers[idx]) === String(req.user.id);
      });
      return res.json(filtered);
    } else {
      // list all company expenses (admins/managers) or own expenses for employee
      if (req.user.role === 'Admin' || req.user.role === 'Manager') {
        const all = await Expense.find({ companyId: req.user.companyId }).populate('userId', 'name email').sort({ createdAt: -1 });
        return res.json(all);
      } else {
        const mine = await Expense.find({ userId: req.user.id }).populate('userId', 'name email').sort({ createdAt: -1 });
